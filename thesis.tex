...
\begin{theo} Let $G$ be a $k$-band buffering cellular graph. Consider the graph $G'$ which is the modification of $G$ according to equation \ref{eq:graph-mod} then $G'$ has a node $v$ with $\deg(v) \leqslant 3(k+1)k/2$.
\end{theo}
\begin{proof} Let $G$ be a $k$-band buffering cellular graph and $v$ any node of $G$. It is easy to see that the maximum number of $m$-distance neighbors of $v$ is $6m$. Therefore the maximum number of at most $k$-distance neighbors of $v$ is 
...
$$\deg(v) \leqslant \frac{3(k+1)k}{2} \text{ in $G'$}$$ 
as required.
\begin{figure}[!h]
\centering
...
\end{figure}
\end{proof}
\begin{corollary} Let $G$ be a $k$-band buffering cellular graph then $G$ is $(3(k+1)k/2+1)$-choosable.
\end{corollary}
...
The number of non-overlapping channels with $22$ MHz channel width is $3$ (namely Channel $1, 6$ and $11$) meaning that an interference-free allocation is possible if and only if the interference graph $G$ has a chromatic number of $3$. In practice, this rarely happens due to the growing number of IEEE 802.11 access points which results in a very crowded deployment.
\begin{figure}[!h]
\centering
