...
\abstract{The optimal channel allocation problem in cellular networks is often formulated in a graph theoretic framework. One of its variants\---where each access point knows the list of its free channels\---is related the so-called list coloring problem and is closely related to the channel allocation of IEEE 802.11 systems. In spite of the fact that the list coloring problem is NP-complete for arbitrary graphs, we show that there exists a polynomial time algorithm that $l$-list colors $k$-band buffering callular graphs where $l$ is the Szekeres\---Wilf number of the graph. In addition, an upper bound for the choice number of such graph is obtained proving that $k$-band buffering cellular graphs are $(3k(k+1)/2+1)$-choosable.

A Java application is implemented to compute the Szekeres\---Wilf number of randomly generated cellular graph in order to facilitate making further conjunctures of sharper upper bounds for the choice number.
...
\section{The $k$-band buffering case and its application in IEEE 802.11 systems}
...
\end{figure}
\end{proof}
\begin{corollary} Let $G^k$ be a $k$-band buffering cellular graph then $G^k$ is $(3(k+1)k/2+1)$-choosable.
\end{corollary}
...
Now, we have everything to modify Algorithm \ref{alg:four-list-coloring} such that it handles $k$-band buffering graphs. Algorithm \ref{alg:szekeres-list-coloring} accepts lists of size $3k(k+1)/2+1$, however it is possible run Algorithm \ref{alg:szekeres-wilf-computation} to precompute the Szekeres\---Wilf number of the current $k$-band buffering graph thereby decreasing the initial list sizes.

\begin{algorithm}\label{alg:szekeres-list-coloring}
 \KwData{A $k$-band buffering cellular graph $G$ and a set of colors $L(v)$ for all $v \in V(G)$ with $|L(v)| \geqslant 3k(k+1)/2+1$}
 \KwResult{$l$-list coloring of $G$ where $l$ is the Szekeres\---Wilf number of $G$}
  Let $G^*$ be the $l$-bounded acyclic orientation of $G$  (Algorithm \ref{alg:find-acyclic-bounded-orientation-general})\;
  \While{$V(G^*)$ is non-empty} {
  	Let $\alpha$ be a color from $L(v)$ where $v \in V(G^*)$\;
  	$V_\alpha := \lbrace v \in V(G) \mid \alpha \in L(v) \rbrace$\;
  	Let $D$ be the subgraph of $G^*$ that $V_\alpha$ spans\;
 ...
 \caption{$l$-list coloring of a $k$-band buffering cellular graph where $l$ is the Szekeres\---Wilf number of $G$}
\end{algorithm}
...
However, this CM can be used to identify the free channels (e.g. if they are below some constant) $L(v)$ at each access point $v$. A controller\---which knows the topology of the access points\---whould be able to run Algorithm \ref{alg:four-list-coloring} along with Algorithm \ref{alg:find-acyclic-bounded-orientation-general}. In addition, using the concept of graph powering we would be able to require $k$-band buffering interference-free allocations if there are enough channels (i.e. more than the Szekeres\---Wilf number of the access point topology graph).
\section{Evaluations}
\begin{table}[h!]
\centering
\begin{tabular}{|c|c|c|}
